...
DOCS_CMDS="markdown table --no-header --no-providers --sort-by-required"
DOCS_DIR=docs
...
function main_docs {
    ...
    eval "${BINARY_FILE} ${DOCS_CMDS} . --no-outputs"  >> "${DOCS_DIR}/${INS_MD}"
    eval "${BINARY_FILE} ${DOCS_CMDS} . --no-inputs" >> "${DOCS_DIR}/${OUTS_MD}"
...
function module_docs {
    if test -d ./modules; then
        for dir in ./modules/*; do
            ...
            eval "${BINARY_FILE} ${DOCS_CMDS} ${dir} --no-outputs"  >> "${dir}/${DOCS_DIR}/${INS_MD}"
            eval "${BINARY_FILE} ${DOCS_CMDS} ${dir} --no-inputs" >> "${dir}/${DOCS_DIR}/${OUTS_MD}"
        done
    else
