variable "ca_certificate_secret_id" {
  description = <<-EOD
  The identity of the Secret Manager secret which comprises the Base64 encoded PEM CA certificate for Terraform
  Enterprise and  mitmproxy.
  EOD
  type        = string
}
...
variable "ca_private_key_secret_id" {
  description = <<-EOD
  The identity of the Secret Manager secret which comprises the Base64 encoded PEM CA private key for mitmproxy.
  EOD
  type        = string
}
